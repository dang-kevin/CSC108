...
def get_total(index: int, stations: List[list]) -> int:
    ...
    for item in stations:
        if item[index] == int(item[index]):
            total.append(item[index])
    return sum(total)
...
def get_station_with_max_bikes(stations: List[list]) -> int:
    ...
    for item in stations:
        if item[BIKES_AVAILABLE] > max_num:
            max_num = item[BIKES_AVAILABLE]
            station_id.append(item[ID])
    return max(station_id)
...
def get_stations_with_n_docks(n: int, stations: List[list]) -> List[int]:
    ...
    for item in stations:
        if item[DOCKS_AVAILABLE] >= n:
            docks.append(item[ID])
    return docks
...
def rent_bike(station_id: int, stations: List[list]) -> bool:
    ...
    for item in stations:
        if station_id in item:
            if item[BIKES_AVAILABLE] > 0 and item[IS_RENTING]:
                item[BIKES_AVAILABLE] -= 1
                item[DOCKS_AVAILABLE] += 1
                return True
            return False
...
def balance_all_bikes(stations: List[list]) -> int:
    ...
    for item in stations:
        ...
        total_bikes_available += item[BIKES_AVAILABLE]

    total_percent = total_bikes_available / total_capacity

    bikes_rented = 0
    bikes_returned = 0

    for item in stations:
        target = round(total_percent * item[CAPACITY])
        change = target - item[BIKES_AVAILABLE]
        if change > 0 and item[IS_RETURNING] and item[DOCKS_AVAILABLE] > 0:
            bikes_returned = abs(change)
            item[BIKES_AVAILABLE] += change
            item[DOCKS_AVAILABLE] -= change
        elif change < 0 and item[IS_RENTING] and item[BIKES_AVAILABLE] > 0:
            bikes_rented = abs(change)
            item[BIKES_AVAILABLE] += change
            item[DOCKS_AVAILABLE] -= change
        else:
            pass
